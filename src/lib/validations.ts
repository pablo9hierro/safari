...
export const serviceRequestSchema = z.object({
  ...
  diagnosis_requested:  z.boolean().optional(),
  ...
  if (!data.diagnosis_requested && (!data.phone_model || data.phone_model.length < 2)) {
    ctx.addIssue({ code: 'custom', path: ['phone_model'], message: 'Informe o modelo do celular' })
  }
