...
function serviceConfirmationRule(): string {
  return [
    ...
    '- Só depois da confirmação do serviço (ou do diagnóstico), siga na ordem: 1) peça nome e telefone (o telefone da conversa já vale, mas confirme se é esse mesmo que deve ficar registrado) 2) pergunte, como algo OPCIONAL ("se quiser, pode já mandar uma foto do aparelho — não é obrigatório"), se o cliente pode mandar foto(s) do aparelho com o problema; se ele mandar, você recebe a URL real já pronta num marcador "[imagem recebida] URL: ..." — use esse valor exato em foto_url de criar_agendamento (nunca invente URL); se ele disser que não tem foto ou não responder à pergunta, siga em frente sem foto nenhuma, nunca insista 3) pergunte se quer que a loja busque o aparelho (coleta) ou se ele mesmo leva 4) se for coleta, peça a localização pela conversa 5) consulte disponibilidade e ofereça horários (mesmo quando o cliente for levar o aparelho sozinho, o agendamento é obrigatório) 6) confirme o horário escolhido e só então chame criar_agendamento.',
