    ...
    <div className="space-y-4">
      <form onSubmit={handleCreate} className="bg-vr-graphite rounded-2xl border border-white/5 p-4 space-y-3">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            ...
            <input type="number" step="0.01" min="0" value={newQuantity} onChange={(e) => setNewQuantity(e.target.value)} placeholder="0"
              className="w-full mt-1 px-3 py-2.5 rounded-xl bg-vr-black border border-white/10 text-white placeholder-vr-silver/40 text-sm outline-none focus:border-vr-red" />
          </div>
          <div>
            <label className="text-xs font-semibold text-vr-silver/60 uppercase tracking-wide">Unidade *</label>
            <select value={newUnit} onChange={(e) => setNewUnit(e.target.value as StockUnit)}
      ...
      <div className="space-y-2">
        ...
        <div className="relative">
          ...
          <input value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Buscar item..."
        ...
        {filteredItems.length === 0 ? (
          <p className="text-sm text-vr-silver/40">{items.length === 0 ? 'Nenhum item cadastrado ainda.' : 'Nenhum item encontrado.'}</p>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            {filteredItems.map((item) => (
              <div key={item.id} role="button" tabIndex={0} onClick={() => openActions(item)}
              ...
              >
                ...
                <span className="flex items-center gap-2 shrink-0">
                  ...
                  <button type="button" onClick={(e) => { e.stopPropagation(); openEdit(item) }} className="text-vr-silver/40 hover:text-vr-red transition-colors p-1">
            ...
            {movements.map((m) => (
              <li key={m.id} className="bg-vr-graphite rounded-xl border border-white/5 px-4 py-2.5 flex items-center justify-between gap-3">
                <div className="flex items-center gap-2 min-w-0">
                  {m.type === 'entrada' ? <ArrowDownCircle className="w-4 h-4 text-green-500 shrink-0" /> : <ArrowUpCircle className="w-4 h-4 text-vr-red shrink-0" />}
                  <span className="text-sm text-white truncate">{m.stock_items?.name ?? 'Item removido'}</span>
                </div>
                <div className="flex items-center gap-3 shrink-0">
                  <span className="text-sm font-semibold text-vr-silver">{m.type === 'entrada' ? '+' : '-'}{Number(m.quantity)} {m.unit}</span>
                  <span className="text-xs text-vr-silver/40">{new Date(m.moved_at).toLocaleString('pt-BR', { day: '2-digit', month: '2-digit', hour: '2-digit', minute: '2-digit' })}</span>
                </div>
              </li>
            ))}
          </ul>
      ...
      {/* Popup: ações */}
      {actionsItem && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-end sm:items-center justify-center p-0 sm:p-4" onClick={closeActions}>
          <div className="bg-vr-graphite w-full sm:max-w-sm sm:rounded-2xl rounded-t-2xl border border-white/10 p-5 space-y-3" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between gap-3">
              ...
              <button onClick={closeActions} className="text-vr-silver/50 hover:text-white shrink-0"><X className="w-4 h-4" /></button>
            </div>
            {confirmingDelete ? (
              ...
              <div className="space-y-2">
                <button onClick={() => { openExit(actionsItem); closeActions() }}
                  ...
                  <ArrowUpCircle className="w-4 h-4 text-vr-red" /> Registrar saída
                </button>
                <button onClick={() => { openEdit(actionsItem); closeActions() }}
                  className="w-full flex items-center gap-2.5 text-sm font-semibold text-white bg-vr-black border border-white/10 rounded-xl px-4 py-3 hover:border-vr-red/40 transition-colors">
                  <Pencil className="w-4 h-4 text-vr-red" /> Editar item
                </button>
      ...
      {/* Popup: editar */}
      {editItem && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-end sm:items-center justify-center p-0 sm:p-4" onClick={closeEdit}>
          <div className="bg-vr-graphite w-full sm:max-w-sm sm:rounded-2xl rounded-t-2xl border border-white/10 p-5 space-y-3" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between">
              <h3 className="text-base font-bold text-white">Editar item</h3>
              <button onClick={closeEdit} className="text-vr-silver/50 hover:text-white"><X className="w-4 h-4" /></button>
            </div>
            <div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="text-xs font-semibold text-vr-silver/60 uppercase tracking-wide">Quantidade *</label>
                <input type="number" step="0.01" min="0" value={editQuantity} onChange={(e) => setEditQuantity(e.target.value)}
                  className="w-full mt-1 px-3 py-2.5 rounded-xl bg-vr-black border border-white/10 text-white text-sm outline-none focus:border-vr-red" />
              </div>
              <div>
                <label className="text-xs font-semibold text-vr-silver/60 uppercase tracking-wide">Unidade *</label>
                <select value={editUnit} onChange={(e) => setEditUnit(e.target.value as StockUnit)}
                  className="w-full mt-1 px-3 py-2.5 rounded-xl bg-vr-black border border-white/10 text-white text-sm outline-none focus:border-vr-red">
                  <option value="unidade">Unidade</option>
                  <option value="caixa">Caixa</option>
                </select>
      ...
      {/* Popup: saída */}
      {exitItem && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-end sm:items-center justify-center p-0 sm:p-4" onClick={closeExit}>
          <div className="bg-vr-graphite w-full sm:max-w-sm sm:rounded-2xl rounded-t-2xl border border-white/10 p-5 space-y-3" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between gap-3">
              <div className="min-w-0">
                <h3 className="text-base font-bold text-white">Registrar saída</h3>
                <p className="text-xs text-vr-silver/50 truncate">{exitItem.name} — {Number(exitItem.quantity)} {exitItem.unit}</p>
              </div>
              <button onClick={closeExit} className="text-vr-silver/50 hover:text-white shrink-0"><X className="w-4 h-4" /></button>
            </div>
            <div>
              <label className="text-xs font-semibold text-vr-silver/60 uppercase tracking-wide">Quantidade de saída *</label>
              <input type="number" step="0.01" min="0.01" value={exitQuantity} onChange={(e) => setExitQuantity(e.target.value)} placeholder="0"
                className="w-full mt-1 px-3 py-2.5 rounded-xl bg-vr-black border border-white/10 text-white placeholder-vr-silver/40 text-sm outline-none focus:border-vr-red" />
            </div>
            {exitError && <p className="text-xs text-red-400">{exitError}</p>}
            <button onClick={handleRegisterExit} disabled={savingExit} className="btn-primary w-full flex items-center justify-center gap-2">
